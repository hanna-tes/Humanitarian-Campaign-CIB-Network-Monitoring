...
def combine_social_media_data(
    ...
    if openmeasure_df is not None and not openmeasure_df.empty:
        ...
        om['object_id'] = get_specific_col(om, 'text')
...
with tab1:
    ...
    st.info("This tab provides a high-level overview of the uploaded datasets, including total posts, a breakdown by platform, and the date range of the data.")

    col1, col2, col3 = st.columns(3)
    col1.metric("Total Posts Analyzed", f"{len(df_for_analysis):,}")
    
    if 'timestamp_share' in df_for_analysis.columns and not df_for_analysis.empty:
    ...
    if 'account_id' in df_for_analysis.columns:
        col3.metric("Total Unique Accounts", f"{df_for_analysis['account_id'].nunique():,}")

    st.markdown("---")

    st.subheader("Posts by Platform")
    if 'Platform' in df_for_analysis.columns and not df_for_analysis.empty:
        platform_counts = df_for_analysis['Platform'].value_counts().reset_index()
        ...
        fig = px.bar(
            ...
            labels={'Platform': 'Platform', 'Count': 'Number of Posts'},
            color='Platform'
        )
...
with tab2:
    ...
    if coordinated_groups:
        ...
        st.info("Please find coordinated groups in the 'Coordination Analysis' tab first.")
    ...
    if 'URL' in df_for_analysis.columns and not df_for_analysis['URL'].isnull().all():
        ...
        df_urls['Domain'] = df_urls['URL'].apply(lambda x: tldextract.extract(str(x)).domain + '.' + tldextract.extract(str(x)).suffix)
...
with tab4:
    ...
    with col1:
        ...
        if not df_for_phrases.empty:
            ...
            df_phrases = pd.DataFrame(phrase_counts.items(), columns=['Phrase', 'Count']).sort_values('Count', ascending=False)
    ...
    if not fundraising_df.empty:
        ...
        fundraising_df['domain'] = fundraising_df['donation_link'].apply(lambda x: tldextract.extract(x).domain if pd.notna(x) else 'Unknown')
